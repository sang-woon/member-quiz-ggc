...
Task tool parameters:
- subagent_type: "backend-specialist"
- description: "Phase 1, T1.1: 의원 API 구현"
- prompt: |
    ## 태스크 정보
    - Phase: 1
    - 태스크 ID: T1.1
    ...
    ## TDD 요구사항 (Phase 1+ 필수!)
    반드시 TDD 사이클을 따르세요:
    1. RED: 테스트 먼저 작성 (tests/api/test_members.py)
    2. GREEN: 테스트 통과하는 최소 구현
    3. REFACTOR: 테스트 유지하며 코드 정리

    테스트 명령어: `pytest tests/api/test_members.py -v`
    ...
    - 완료 보고 형식에 맞춰 보고
    - 사용자 승인 후에만 main 병합
    - 병합 후 worktree 정리: `git worktree remove ../member-quiz-phase1-api`
```
...
| `database-specialist` | SQLAlchemy, SQLite, 스키마 설계 |
